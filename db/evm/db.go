...
type TokenDB interface {
	...
	GetTokenList(contractAddr, ownerAddr string, ids []int64) ([]*Token, error)
	GetTokenMap(contractAddr, ownerAddr string, ids []int64) (map[int64]*Token, error)
}
...
func TokenID(contractAddr, ownerAddr string, goodsID int64) string {
	return fmt.Sprintf("%s-%s-%s-%v", "token", contractAddr, ownerAddr, goodsID)
}

// GetTokenList 获取某用户的多个id通证
func (d *tokenDB) GetTokenList(contractAddr, ownerAddr string, ids []int64) ([]*Token, error) {
...
func (d *tokenDB) GetTokenMap(contractAddr, ownerAddr string, ids []int64) (map[int64]*Token, error) {
	tokens, err := d.GetTokenList(contractAddr, ownerAddr, ids)
	if err != nil {
		return nil, err
	}

	ans := make(map[int64]*Token)

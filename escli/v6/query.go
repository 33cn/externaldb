...
func queryShouldHelper(q *elasticV6.BoolQuery, s *querypara.QMatch) *elasticV6.BoolQuery {
	if s.Value == nil {
		qi := elasticV6.NewExistsQuery(s.Key)
		return q.Should(qi)
	}
	if QueryFunc == "match_phase" {
		qi := elasticV6.NewMatchPhraseQuery(s.Key, s.Value)
		return q.Should(qi)
	} else if QueryFunc == "term" {
		qi := elasticV6.NewTermQuery(s.Key, s.Value)
		return q.Should(qi)
	}
	qi := elasticV6.NewMatchQuery(s.Key, s.Value)
	return q.Should(qi)
}

func queryMustHelper(q *elasticV6.BoolQuery, s *querypara.QMatch) *elasticV6.BoolQuery {
	if s.Value == nil {
		qi := elasticV6.NewExistsQuery(s.Key)
		return q.Must(qi)
	}
...
func queryMustNotHelper(q *elasticV6.BoolQuery, s *querypara.QMatch) *elasticV6.BoolQuery {
	if s.Value == nil {
		qi := elasticV6.NewExistsQuery(s.Key)
		return q.MustNot(qi)
	}
...
func queryFilterHelper(q *elasticV6.BoolQuery, s *querypara.QMatch) *elasticV6.BoolQuery {
	if s.Value == nil {
		qi := elasticV6.NewExistsQuery(s.Key)
		return q.Filter(qi)
	}
...
// ShouldMatch 最少匹配一项
func addShouldMatch(query *querypara.Query, q *elasticV6.BoolQuery) *elasticV6.BoolQuery {
...
func addMultiMatch(query *querypara.Query, q *elasticV6.BoolQuery) *elasticV6.BoolQuery {
	if query.MultiMatch != nil && len(query.MultiMatch) > 0 {
		for _, s := range query.MultiMatch {
			qi := elasticV6.NewMultiMatchQuery(s.Value, s.Keys...)
			q = q.Should(qi)
		}
		matchCount := 1
		if query.Match != nil {
...
func addMatch(query *querypara.Query, q *elasticV6.BoolQuery) *elasticV6.BoolQuery {
...
func addMatchNot(query *querypara.Query, q *elasticV6.BoolQuery) *elasticV6.BoolQuery {
...
func addFilterMatch(query *querypara.Query, q *elasticV6.BoolQuery) *elasticV6.BoolQuery {
...
func setRange(query *querypara.Query, q *elasticV6.BoolQuery) *elasticV6.BoolQuery {
	if query.Range != nil && len(query.Range) > 0 {
		for _, s := range query.Range {
			qi := elasticV6.NewRangeQuery(s.Key)
			if s.RStart != nil {
				qi = qi.From(s.RStart)
			}
			if s.REnd != nil {
...
func getQuery(query *querypara.Query) *elasticV6.BoolQuery {
	if query == nil {
		return nil
	}
...
func Query(search *elasticV6.SearchService, query *querypara.Query) (*elasticV6.SearchService, error) {
	if query == nil {
	...
	if query.Fetch != nil && len(query.Fetch.Keys) > 0 {
		fsc := elasticV6.NewFetchSourceContext(query.Fetch.FetchSource).Include(query.Fetch.Keys...)
